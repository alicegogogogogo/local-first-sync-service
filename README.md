...
附件的授权名单查询入口（只读）：列出当前仍被授权读取该附件的全部设备标识。无请求体，也不引入新的认证机制——路径上的设备标识即调用者身份。

- 返回 `200` `{"devices":["device-2", ...]}`。名单只含当前仍被授权的设备，按最近一次授权生效的先后升序稳定排列；同一设备最多出现一次，即使它曾被撤回后重新授权（此时按最新一次授权的位置排列）。无授权设备时为 `[]`。
- 分页口径与设备附件清单一致：`limit` 为 1..1000 的整数（缺省 100），`offset` 为非负整数（缺省 0）；取值非法返回 `400` JSON 错误。同一设备在两次读取之间授权没有增删时，翻页得到的条目不重不漏。
- 仅创建者可查询：路径设备不是创建者返回 `403` JSON 错误且不泄露名单内容；附件不存在或已删除一律返回 `404` JSON 错误，两个判定相互独立。
- 分页参数非法、标识为空、路径段缺失或多余、方法不是 `GET`（`POST` 授权变更除外）一律返回 `400` JSON 错误，不重定向也不输出 HTML；这些失败都不写入或改动任何附件与授权记录。
- 撤回某设备的授权后它立即从名单消失，创建者的其他授权与附件内容不受影响；附件删除后该入口对任何人稳定返回 `404`。
- 名单是只读入口：查询不创建、修改或删除授权，也不改变去重与封存判定。授权状态与名单顺序同步落盘，进程重启后同一请求的响应逐字不变。
