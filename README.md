...
- 附件一旦成功封存即为不可变：此后对任何序号的分块写入——即使字节与已保存内容完全相同——一律返回 `409` JSON 错误，封存状态、`reused` 标记与已保存内容均不改变。
...
- 成功返回 `200`：`{"attachmentId","size","sha256","complete":true,"reused":...}`；重复封存返回首次封存的完整结果，不重新计算、不改变 `reused` 标记。封存成功后附件不可变，任何分块写入均为 `409`。
...
附件的创建、分块、完成与读取均同步落盘；封存状态、内容去重与封存后的拒写判定也一并持久化，进程重启后上传状态、去重结果、幂等判定以及封存后写入拒绝均保持一致。
