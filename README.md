...
在变更日志之外提交一批 CRDT 操作，驱动文档可自动合并的 CRDT 状态。仅接受 `Content-Type: application/json`。请求体声明文档类型（`"counter"` 计数器、`"gset"` 只增集合或 `"register"` 单值寄存器）：
...
只增集合的操作形如 `{"id":"op-1","elements":["apple","banana"]}`；寄存器的操作形如 `{"id":"op-1","version":3,"value":<任意JSON>}`。

- 文档类型由**第一次成功提交**确定为计数器、只增集合或寄存器，此后不可更改；之后批次声明其他类型返回 `409` JSON 错误。并发声明不同类型时在序列化事务内恰好一个生效，其余请求返回 `409`，双方都在同一条数据库连接上串行判定。
...
- 寄存器：操作携带**非负整数逻辑版本** `version` 与任意合法 JSON `value`（`null`、数字、字符串、数组或对象均可，按原样保存）。同一设备后续提交的版本必须严格大于此前已接受的版本，倒退或持平返回 `409` 且状态不变。合并状态取逻辑版本最大的操作的值，版本相同时按操作 `id` 字典序取较小者的值；合并结果与到达顺序无关（交换、结合、幂等）。
- 每个操作带稳定的非空字符串 `id`，在文档内唯一；`ops` 必须是非空数组，批内 `id` 重复返回 `400`。重复提交同一 `id`：设备与内容都相同视为幂等（`created=false`，计数器按值比较、集合按元素集合比较且与元素顺序无关、寄存器按值与版本比较）；内容或来源设备不同返回 `409` 且状态不变。
...
- 寄存器返回 `200` `{"type":"register","value":<JSON>}`，`value` 原样呈现当前被选中的 JSON 内容（可以是 `null`、数字、字符串、数组或对象）。
...
成功正文与文档级状态读取**逐字一致**：紧凑单行 JSON，键按 `type`、`value` 顺序，末尾带一个换行——计数器为 `{"type":"counter","value":N}`（`N` 为各设备最大贡献之和，非负整数），只增集合为 `{"type":"gset","value":[...]}`（全部元素的升序数组），寄存器为 `{"type":"register","value":<JSON>}`（当前被选中的 JSON 内容，原样呈现）。任意时刻该正文都与订阅推送消息、`GET /v1/documents/{documentID}/crdt/state` 的结果完全一致。
...
- 计数器各设备最大贡献之和一旦变化、只增集合的并集一旦增长、或寄存器的当选操作一旦易主，就在事务提交后立即把新的合并结果推送给该文档的全部订阅方；先后顺序以事务提交顺序为准。
- 幂等的重复提交、被拒绝的倒退推进或版本回退（`409`）和整批非法请求（`400`）都不改状态，也不产生任何通知；向只增集合重复添加已有元素、或寄存器新操作未能当选（版本不够大）都不算状态变化，只有合并值真正变化时才推送一条消息。
- 每条消息是一个文本帧，内容与 `GET .../crdt/state` 的正文逐字一致：紧凑单行 JSON，键按 `type`、`value` 顺序，末尾带一个换行。计数器消息为 `{"type":"counter","value":N}\n`（N 为各设备最大贡献之和），只增集合消息为 `{"type":"gset","value":[...]}\n`（元素升序），寄存器消息为 `{"type":"register","value":<JSON>}\n`（当选操作的 JSON 内容，原样呈现），计数器数值一律非负整数。
