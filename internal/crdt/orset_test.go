...
func orsetOp(id, device, action, element string) crdt.Op {
	return crdt.Op{ID: id, DeviceID: device, Action: action, Element: element}
}

func submitORSet(t *testing.T, s *app.App, doc string, ops ...crdt.Op) []crdt.Result {
	t.Helper()
	results, err := s.SubmitCRDTOps(doc, crdt.TypeORSet, ops)
	if err != nil {
...
func orsetState(t *testing.T, s *app.App, doc string) []string {
	t.Helper()
	state, err := s.GetCRDTState(doc)
	if err != nil {
		t.Fatal(err)
	}
	if state.Type != crdt.TypeORSet {
...
func TestCRDTORSetAddAndRemove(t *testing.T) {
	...
	submitORSet(t, s, "doc", orsetOp("a1", "dev-1", crdt.ORSetAdd, "banana"))
	submitORSet(t, s, "doc", orsetOp("a2", "dev-1", crdt.ORSetAdd, "apple"))
	submitORSet(t, s, "doc", orsetOp("b1", "dev-2", crdt.ORSetAdd, "cherry"))
	equalElements(t, orsetState(t, s, "doc"), "apple", "banana", "cherry")

	// A remove deletes exactly the adds it observes.
	submitORSet(t, s, "doc", orsetOp("r1", "dev-2", crdt.ORSetRemove, "banana"))
	equalElements(t, orsetState(t, s, "doc"), "apple", "cherry")

	// Removing the last element leaves an empty set, rendered as [].
	submitORSet(t, s, "doc",
		orsetOp("r2", "dev-1", crdt.ORSetRemove, "apple"),
		orsetOp("r3", "dev-1", crdt.ORSetRemove, "cherry"),
	)
...
func TestCRDTORSetRemoveOnlyObservedAdds(t *testing.T) {
	...
	submitORSet(t, s, "doc", orsetOp("a1", "dev-1", crdt.ORSetAdd, "x"))
	submitORSet(t, s, "doc", orsetOp("r1", "dev-2", crdt.ORSetRemove, "x"))
	equalElements(t, orsetState(t, s, "doc"))

	// An add landing after the remove carries a fresh tag and survives it.
	submitORSet(t, s, "doc", orsetOp("a2", "dev-1", crdt.ORSetAdd, "x"))
	...
	submitORSet(t, s, "doc", orsetOp("a3", "dev-2", crdt.ORSetAdd, "y"))
	submitORSet(t, s, "doc", orsetOp("r2", "dev-1", crdt.ORSetRemove, "y"))
	submitORSet(t, s, "doc", orsetOp("a4", "dev-2", crdt.ORSetAdd, "y"))
...
func TestCRDTORSetRemoveNeverAddedIsNoOp(t *testing.T) {
	...
	results := submitORSet(t, s, "doc", orsetOp("r1", "dev-1", crdt.ORSetRemove, "ghost"))
	...
	submitORSet(t, s, "doc", orsetOp("a1", "dev-1", crdt.ORSetAdd, "ghost"))
...
func TestCRDTORSetIdempotencyAndConflict(t *testing.T) {
	...
	results := submitORSet(t, s, "doc", orsetOp("a1", "dev-1", crdt.ORSetAdd, "x"))
	if results[0].Created {
	...
	if _, err := s.SubmitCRDTOps("doc", crdt.TypeORSet, []crdt.Op{
		orsetOp("a1", "dev-1", crdt.ORSetAdd, "y"),
	}); !errors.As(err, &conflict) || conflict.ID != "a1" {
		t.Fatalf("different element err = %v, want *crdt.ErrConflict for a1", err)
	}
	// Same id, different action -> 409.
	if _, err := s.SubmitCRDTOps("doc", crdt.TypeORSet, []crdt.Op{
		orsetOp("a1", "dev-1", crdt.ORSetRemove, "x"),
	}); !errors.As(err, &conflict) {
		t.Fatalf("different action err = %v, want *crdt.ErrConflict", err)
	}
	// Same id, different device -> 409.
	if _, err := s.SubmitCRDTOps("doc", crdt.TypeORSet, []crdt.Op{
		orsetOp("a1", "dev-2", crdt.ORSetAdd, "x"),
	}); !errors.As(err, &conflict) {
		t.Fatalf("different device err = %v, want *crdt.ErrConflict", err)
	}
	equalElements(t, orsetState(t, s, "doc"), "x")

	// A replayed remove is idempotent and must not tombstone adds accepted
	// after its first acceptance.
	submitORSet(t, s, "doc", orsetOp("r1", "dev-1", crdt.ORSetRemove, "x"))
	submitORSet(t, s, "doc", orsetOp("a2", "dev-1", crdt.ORSetAdd, "x"))
	results = submitORSet(t, s, "doc", orsetOp("r1", "dev-1", crdt.ORSetRemove, "x"))
	if results[0].Created {
...
func TestCRDTORSetMergeIsOrderIndependent(t *testing.T) {
	...
	submitORSet(t, s, "doc-a",
		orsetOp("a1", "dev-1", crdt.ORSetAdd, "x"),
		orsetOp("a2", "dev-2", crdt.ORSetAdd, "y"),
		orsetOp("a3", "dev-1", crdt.ORSetAdd, "z"),
	)
	submitORSet(t, s, "doc-b",
		orsetOp("a3", "dev-1", crdt.ORSetAdd, "z"),
		orsetOp("a1", "dev-1", crdt.ORSetAdd, "x"),
		orsetOp("a3", "dev-1", crdt.ORSetAdd, "z"), // idempotent replay
		orsetOp("a2", "dev-2", crdt.ORSetAdd, "y"),
		orsetOp("a1", "dev-1", crdt.ORSetAdd, "x"), // idempotent replay
	)
...
func TestCRDTORSetTypeFixation(t *testing.T) {
	...
	if _, err := s.SubmitCRDTOps("doc", crdt.TypeCounter, counterOps("dev-1", 1)); !errors.As(err, &conflict) {
	...
	if _, err := s.SubmitCRDTOps("other", crdt.TypeGSet, []crdt.Op{
		{ID: "g1", DeviceID: "dev-1", Elements: []string{"x"}},
	}); err != nil {
		t.Fatal(err)
	}
	if _, err := s.SubmitCRDTOps("other", crdt.TypeORSet, []crdt.Op{
		orsetOp("o1", "dev-1", crdt.ORSetAdd, "x"),
	}); !errors.As(err, &conflict) {
...
func TestCRDTORSetNotificationOnlyOnRealChange(t *testing.T) {
	...
	submitORSet(t, s, "doc", orsetOp("r1", "dev-1", crdt.ORSetRemove, "ghost"))
	queued := sub.Drain()
	...
	submitORSet(t, s, "doc", orsetOp("r2", "dev-1", crdt.ORSetRemove, "x"))
	...
	for _, d := range devices {
		go func(device string) {
			...
			for e := 0; e < elements; e++ {
				if _, err := s.SubmitCRDTOps("doc", crdt.TypeORSet, []crdt.Op{
					orsetOp(fmt.Sprintf("%s-%d", device, e), device, crdt.ORSetAdd, fmt.Sprintf("e%02d", e)),
				}); err != nil {
...
func TestCRDTORSetPersistsAcrossReopen(t *testing.T) {
	...
	registerDevices(t, s, "dev-1", "dev-2")
	submitORSet(t, s, "doc",
		orsetOp("a1", "dev-1", crdt.ORSetAdd, "b"),
		orsetOp("a2", "dev-1", crdt.ORSetAdd, "a"),
	)
	submitORSet(t, s, "doc", orsetOp("r1", "dev-2", crdt.ORSetRemove, "b"))
	submitORSet(t, s, "doc", orsetOp("a3", "dev-2", crdt.ORSetAdd, "b"))
	...
	results, err := s2.SubmitCRDTOps("doc", crdt.TypeORSet, []crdt.Op{
		orsetOp("r1", "dev-2", crdt.ORSetRemove, "b"),
	})
	...
	var conflict *crdt.ErrConflict
	if _, err := s2.SubmitCRDTOps("doc", crdt.TypeORSet, []crdt.Op{
		orsetOp("a1", "dev-1", crdt.ORSetAdd, "different"),
	}); !errors.As(err, &conflict) {
		t.Fatalf("conflict after restart err = %v", err)
	}
	// Type fixation survives.
	if _, err := s2.SubmitCRDTOps("doc", crdt.TypeCounter, counterOps("dev-1", 1)); !errors.As(err, &conflict) {
