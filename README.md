...
会话视角的 WebSocket CRDT 状态订阅（CRDT 状态的通知与推送层）。客户端携带已存在的会话标识与文档标识发起 RFC 6455 升级握手，服务端在握手通过后建立一条**只推不写**的长连接。无新增认证机制：订阅身份完全由已存在的会话标识决定（会话所属设备即为订阅设备）。
...
- 方法不是 `GET`、缺少或不合法的升级握手（`Connection: Upgrade`、`Upgrade: websocket`、`Sec-WebSocket-Version: 13`、合法 `Sec-WebSocket-Key` 任一不满足）返回 `400` JSON 错误，不重定向、不输出 HTML。
- `sessionId`、`documentId` 为空段、路径段缺失或多余（尾斜杠、额外段等）同样返回 `400` JSON 错误。
- 校验顺序固定为：握手形状（`400`）→ 会话存在性（会话不存在或已删除为 `404`）→ 权限（会话所属设备对该文档权限被撤回为 `403`），全部发生在升级之前，错误均为 JSON 且不暴露任何 CRDT 类型或状态。
- 握手成功返回 `101 Switching Protocols` 与按 RFC 6455 计算的 `Sec-WebSocket-Accept`。

连接建立后的推送语义：

- 文档已有 CRDT 状态时，握手后先推一条**当前合并状态**；文档尚无任何 CRDT 操作时不推送、静默等待，首个状态出现时再推第一条。
- 计数器或只增集合的合并结果一旦变化，就在事务提交后立即推送给该文档的所有订阅方；先后顺序以事务提交顺序为准。幂等的重复提交、被拒绝的倒退推进、等值贡献以及向集合重复添加已有元素都不改变合并值，不产生任何通知；整批非法请求（`400`/`409`）不提交也不通知。
- 每条消息是一个文本帧，内容与状态读取入口的正文逐字一致：紧凑单行 JSON，键按 `type`、`value` 顺序，末尾带一个换行。计数器为 `{"type":"counter","value":N}\n`（`N` 为各设备最大贡献之和），只增集合为 `{"type":"gset","value":[...]}\n`（升序元素数组，数值一律非负整数）。
- 推送不占用文档游标，不产生变更记录或操作记录，也不改变幂等判定；收到的状态随时可与 `GET .../crdt/state` 的结果对照。
- 连接只推不写：客户端经该连接发送的任何数据帧都会被读取并丢弃，不能提交或修改任何 CRDT 操作；客户端的 `ping` 会收到 `pong`，`close` 按 RFC 6455 回应。

连接生命周期与关闭码：

- 订阅建立后权限被撤回时，服务端以关闭码 **4403** 结束订阅，随后停止推送任何状态；该撤回在本连接上是一次性且不可撤销的——即使紧接着重新授权，本连接仍以 4403 结束（客户端需以当前会话重新订阅）。
- 收到终止信号（`SIGINT`/`SIGTERM`）时，服务端以关闭码 **1001**（going away）结束全部订阅；已提交的状态保持可读，进程以零状态退出。终止信号优先于同时挂起的撤回。
- 客户端断开或网络中断时服务端立即释放订阅，不留下任何记录。订阅状态不持久化：进程重启后原订阅消失，重新订阅会立即拿到与状态读取入口一致的当前合并状态（尚无操作的文档仍静默等待首个状态）。
